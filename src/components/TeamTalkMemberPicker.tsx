    ...
    <div className="space-y-3">
      ...
      {hasAnySelection && (
        <div className="flex flex-wrap gap-1.5">
          {value.outlets.map(id => {
            ...
            return (
              <span key={`o-${id}`} className="inline-flex items-center gap-1 pl-2 pr-1 py-1 rounded-full bg-amber-100 text-amber-800 text-[13px] font-semibold">
          ...
          {value.depts.map(d => (
            <span key={`d-${d}`} className="inline-flex items-center gap-1 pl-2 pr-1 py-1 rounded-full bg-emerald-100 text-emerald-800 text-[13px] font-semibold">
          ...
          {value.roles.map(r => (
            <span key={`r-${r}`} className="inline-flex items-center gap-1 pl-2 pr-1 py-1 rounded-full bg-violet-100 text-violet-800 text-[13px] font-semibold">
          ...
          {value.individuals.map(id => {
            ...
            return (
              <span key={`i-${id}`} className="inline-flex items-center gap-1 pl-2 pr-1 py-1 rounded-full bg-slate-100 text-slate-700 text-[13px] font-semibold">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-[15px] text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#162D4E]/20"
        />
      </div>
      ...
      {q && (
        <div className="border border-slate-100 rounded-xl bg-white shadow-sm max-h-56 overflow-y-auto divide-y divide-slate-50">
          {!hasSuggestions && (
            <p className="text-center text-[13px] text-slate-400 p-3">No matches for "{query}"</p>
          )}
          {matchedOutlets.length > 0 && (
            <div className="p-2">
              <p className="text-[11px] font-bold text-amber-600 uppercase tracking-wider px-1 mb-1">Outlets</p>
              {matchedOutlets.map(t => (
                <button key={t.id} onClick={() => addOutlet(t.id)} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-amber-50 text-left cursor-pointer">
                  ...
                  <span className="text-[14px] text-slate-700 font-medium flex-1 truncate">{t.name}</span>
                </button>
              ))}
            </div>
          )}
          {matchedDepts.length > 0 && (
            <div className="p-2">
              <p className="text-[11px] font-bold text-emerald-600 uppercase tracking-wider px-1 mb-1">Departments</p>
              {matchedDepts.map(d => (
                <button key={d} onClick={() => addDept(d)} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-emerald-50 text-left cursor-pointer">
                  ...
                  <span className="text-[14px] text-slate-700 font-medium flex-1 truncate">{d}</span>
                  <span className="text-[12px] text-slate-400">{candidates.filter(u => u.department === d).length}</span>
                </button>
              ))}
            </div>
          )}
          {matchedRoles.length > 0 && (
            <div className="p-2">
              <p className="text-[11px] font-bold text-violet-600 uppercase tracking-wider px-1 mb-1">Roles</p>
              {matchedRoles.map(r => (
                <button key={r} onClick={() => addRole(r)} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-violet-50 text-left cursor-pointer">
                  ...
                  <span className="text-[14px] text-slate-700 font-medium flex-1 truncate">{r}</span>
                  <span className="text-[12px] text-slate-400">{candidates.filter(u => u.role === r).length}</span>
                </button>
              ))}
            </div>
          )}
          {matchedIndividuals.length > 0 && (
            <div className="p-2">
              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider px-1 mb-1">Individuals</p>
              {matchedIndividuals.map(u => (
                <button key={u.id} onClick={() => addIndividual(u.id)} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 text-left cursor-pointer">
                  <div className="w-6 h-6 rounded-full bg-[#162D4E]/10 flex items-center justify-center text-[12px] font-bold text-[#162D4E] shrink-0">{u.name.charAt(0)}</div>
                  <div className="min-w-0">
                    <p className="text-[14px] text-slate-700 font-semibold truncate">{u.name}</p>
                    <p className="text-[11px] text-slate-400 truncate">{u.role} · {u.department}</p>
                  </div>
                </button>
              ))}
            </div>
