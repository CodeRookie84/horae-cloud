/**
 * Full Google Translate language list (free gtx endpoint supports all of these).
 *
 * A client picks a subset of these at onboarding (Client.languages); the in-app
 * translate pickers then offer only that subset, so staff never scroll a
 * 100-language list. `code` is the ISO 639-1 code passed to translateText().
 */
export interface LanguageOption {
  code: string;   // gtx / ISO 639-1 code
...
const BY_CODE: Record<string, LanguageOption> = Object.fromEntries(
  GOOGLE_TRANSLATE_LANGUAGES.map((l) => [l.code, l]),
);
...
/** Resolve a client's stored language codes → full options (unknown codes dropped). */
export function resolveLanguages(codes?: string[] | null): LanguageOption[] {
  return (codes || []).map((c) => BY_CODE[c]).filter(Boolean) as LanguageOption[];
}
